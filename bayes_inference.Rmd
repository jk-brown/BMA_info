...
h_result <- iterate_hector(core, params, save_years = 1990:2100, save_vars = CONCENTRATIONS_CO2())
...
score_hruns_ed <- function(x, criterion, score_function, e,...) {
  ...
  run_wts <- score_function(x_merge, e = e)
...
scored_data <- score_hruns_ed(h_result, criterion_co2_obs(), bayes_wts, e = 2)
...
```{r}
library(ggplot2)
ggplot(data = scored_data, aes(x = RMSE, y = posterior_prob)) +
...
It is also possible to edit the function to use a normal distribution of the observed data as the prior -- this would give higher weight to models that are closer to the distribution of the observed data. Assuming the observed data are are normally distrbuted 
